...
public static class EnumHelper
{
    ...
    public static Dictionary<int, string> GetDictionaryInDisplayName<T>() where T : struct
    {
        if (!typeof(T).IsEnum)
        {
            throw new ArgumentException("T is not an Enum type!");
        ...
        return Enum.GetValues(typeof(T))
            .Cast<object>()
            .ToDictionary(x => (int)x, x => ((Enum)x).GetDisplayName());
    }

    public static Dictionary<int, string> GetDictionaryInDisplayName<T>(params T[] ignoreValues) where T : struct
    {
        if (!typeof(T).IsEnum)
        {
            throw new ArgumentException("T is not an Enum type!");
